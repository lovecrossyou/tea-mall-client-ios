...
class CYLPlusButtonSubclass: CYLPlusButton,CYLPlusButtonSubclassing {
    ...
    override func layoutSubviews() {
        ...
        let imageViewEdgeWidth:CGFloat  = self.bounds.size.width * 1.0
        let imageViewEdgeHeight:CGFloat = imageViewEdgeWidth * 0.8
        
        let centerOfView    = self.bounds.size.width * 0.5
        let labelLineHeight = self.titleLabel!.font.lineHeight
        let verticalMargin = (self.bounds.size.height - labelLineHeight - imageViewEdgeHeight ) * 0.8
        ...
        self.imageView!.center = CGPoint(x:centerOfView, y:centerOfImageView+20)
